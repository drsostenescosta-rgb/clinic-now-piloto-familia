...
- [x] F4. **Operação assistida ponta a ponta** (2026-08-14): o motor que já existia em `medgrowth/emily-vendas` ganhou as três peças que faltavam para uma pessoa não técnica operar — `regras.mjs` (as regras invioláveis da Andreia como código determinístico: a REGRA decide a ação, o LLM só poliria o texto), `voz.mjs` (fala de gente: "drenagem" e não "drenagem linfática", "2 da tarde" e não "14:00", tom que esquenta a partir da 3ª sessão), `ledger.mjs` (aprovações append-only com cadeia de hash: quem aprovou, quando, e o que editou), `api.mjs` (ponte HTTP em 127.0.0.1 com allowlist de origem) e `agendor.mjs` (espelho somente-leitura atrás de flag + comando de descoberta de endpoint). No app: aba **Aprovações** como tela inicial e catálogo real da Andreia no seed. 91 testes, 5/5 cenários críticos, fluxo provado no navegador. Preflight segue REPROVADO com 11 problemas — correto. — aprendizado: rodar de verdade pagou; três defeitos só apareceram em execução (o `undefined` que quebrava a cadeia de hash sem ninguém adulterar nada, a taxa de aprovação sem edição mentindo 0%, e o "Te espero!" numa mensagem de cancelamento).
